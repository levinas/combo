...
DATA_URL = 'http://bioseed.mcs.anl.gov/~fangfang/p1h/'
...
def load_combo_response(min_logconc=-4., max_logconc=-4., subsample=None, fraction=False):
    ...
    df['CELLNAME'] = df.apply(lambda row: df_cellmap.loc[row['CELLNAME'], 'CELLNAME'], axis=1)
    df.rename(columns={'PERCENTGROWTH_MIN': 'GROWTH'}, inplace=True)
